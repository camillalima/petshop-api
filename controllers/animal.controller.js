...
async function getAnimalPorProprietario(req, res, next) {
  try {
    res.send(await AnimalService.getAnimalPorProprietario(req.params.id));
    logger.info('GET /animal/proprietario');
  } catch (err) {
    next(err);
  }
}
